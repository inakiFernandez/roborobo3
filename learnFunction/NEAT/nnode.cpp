...
NNode::NNode (const char *argline, std::vector<Trait*> &traits) {
    ...
    int nodety, nodepl;
    int robotId;
    ss >> robotId >> node_id >> nodety >> nodepl;
...
void NNode::print_to_file(std::ostream &outFile) {
	...
	char tempbuf[128];
    sprintf(tempbuf, "node -1 %d ", node_id);
	outFile << tempbuf;
...
int NNode::depth(int d, Network* mynet, std::vector<NNode*> visited)
{
  ...
  else //Recursion
  {
    for(curlink=innodes.begin();curlink!=innodes.end();++curlink)
    {
        if( // !(*curlink)->is_recurrent
              //  &&
                (std::find(visited.begin(),visited.end(),((*curlink)->in_node))==visited.end()))
        {
            //
            //cur_depth=((*curlink)->in_node)->depth(d+1,mynet);
            cur_depth=((*curlink)->in_node)->depth(d+1,mynet,visited);
            if (cur_depth>max)
                max=cur_depth;
        }
    }
