...
Gene::Gene(const char *argline, std::vector<Trait*> &traits, std::vector<NNode*> &nodes) {
    ...
    ss >> robotId >> inodenum >> robotId >> onodenum >> weight >> recur >> robotId >> innovation_num >> enable;
...
void Gene::print_to_file(std::ofstream &outFile) {
  ...
  outFile<<"-1 " << (lnk->in_node)->node_id<<" ";
  outFile<<"-1 " << (lnk->out_node)->node_id<<" ";
  outFile<<(lnk->weight)<<" ";
  outFile<<(lnk->is_recurrent)<<" ";
  outFile<<"-1 " << innovation_num<<" ";
