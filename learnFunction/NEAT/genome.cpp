...
Genome* Genome::makeGenome(int id, int in,int out)
{
    Genome* g = new Genome(in,out,0,0);
    g->genome_id = id;
    return g;
}
...
/*
	...
	*/
    ...
    {
        //Modify only the newest synapse on multiple synapses
        std::set<std::pair<int,int> > alreadyMut;

        //Reverse Loop on all genes
        std::vector<Gene*>::reverse_iterator rcurgene;
        for(rcurgene=genes.rbegin();rcurgene!=genes.rend();rcurgene++)
        {
            int inNodeID = (*rcurgene)->lnk->in_node->node_id;
            int outNodeID = (*rcurgene)->lnk->out_node->node_id;

            std::pair<int,int> pair = std::make_pair(inNodeID,outNodeID);

            int nb = getNumberSynapses(*rcurgene);

            //Don't mutate weights of frozen links or disabled links
            if (!((*rcurgene)->frozen) && (*rcurgene)->enable)
            {
                //If multisynapse already mutated (only last added synapse)
                //if(alreadyMut.find(pair) == alreadyMut.end())
                {
                    ...
                    //Possible methods of setting the perturbation:
                    //Distribute mutation over all synapses

                    randnum=gaussrand()*power /sqrt((double)nb); //power is standard deviation
                    //randnum=gaussrand();
                    //randnum=randposneg()*randfloat()*power*powermod;
                    ...
                    //Cap the weights at 8.0 (experimental)
                    /*if (((*rcurgene)->lnk)->weight > 8.0) ((*rcurgene)->lnk)->weight = 8.0;
                    else if (((*rcurgene)->lnk)->weight < -8.0) ((*rcurgene)->lnk)->weight = -8.0;*/
                    //REMOVED WEIGHT CAP

                    //Record the innovation
                    //(*rcurgene)->mutation_num+=randnum;
                    (*rcurgene)->mutation_num=((*rcurgene)->lnk)->weight;
                ...
                /*else
                    std::cout << "Already found multi" << std::endl;*/
            }
...
bool Genome::mutate_add_node(std::vector<Innovation*> &innovs,int &curnode_id,double &curinnov) {
	...
	while(!done) {
			...
			((*theinnov)->old_innov_num==(*thegene)->innovation_num)) 
		{
			...
			//Create the new Genes
			if (thelink->is_recurrent) {
                newgene1=new Gene(traitptr,preCoeff ,in_node,newnode,true,(*theinnov)->innovation_num1,0);
                newgene2=new Gene(traitptr,postCoeff * oldweight,newnode,out_node,false,(*theinnov)->innovation_num2,0);
			}
			else {
                newgene1=new Gene(traitptr,preCoeff,in_node,newnode,false,(*theinnov)->innovation_num1,0);
                newgene2=new Gene(traitptr,postCoeff * oldweight,newnode,out_node,false,(*theinnov)->innovation_num2,0);
			}
	...
	if (do_recur) {
		...
		while(trycount<tries) {
            ...
            while (
                 ...
                 (!isExistingConnection || NEAT::allowMultisynapses)
                   )
            {
                ++thegene;
                if(thegene!=genes.end())
	...
	if (found) {
		...
		while(!done) {
			...
			if (theinnov==innovs.end()) {
                ...
                newweight=0.0;//randposneg()*randfloat()*1.0; //used to be 10.0
