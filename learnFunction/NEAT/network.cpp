...
{
    for(std::vector<NNode*>::iterator curnode=all_nodes.begin();curnode!=all_nodes.end();curnode++)
    {
        //Ignore SENSORS
        if ((*curnode)->type!=SENSOR)
        {
...
bool Network::activate()
{
    ...
    //Keep activating until all the neurons become active
    onetime=false;
    for(curnode=all_nodes.begin();curnode!=all_nodes.end();curnode++)
    {
        if ((*curnode)->type!=SENSOR)
            ...
            if ((*curnode)->type!=SENSOR)
            {
                ...
                if ((*curnode)->active_flag)
                {
                    ...
                    if ((*curnode)->ftype==SIGMOID)
                        (*curnode)->activation=NEAT::fsigmoid((*curnode)->activesum,1.0,1.0);
                    else
                    {
...
*/
            ...
            if (((*curnode)->type)!=SENSOR)
            {
                ...
                if ((*curnode)->active_flag)
                {
                    ...
                    if ((*curnode)->overridden())
                    {
						...
						//Now run the net activation through an activation function
						if ((*curnode)->ftype==SIGMOID)
                            (*curnode)->activation=NEAT::fsigmoid((*curnode)->activesum,1.0,1.0);  //Tanh
					}
    ...
    //NOTE: adaptable removed (not used anyway)
...
int Network::max_depth()
{
  ...
  {
    std::vector<NNode*> visited;
    cur_depth=(*curoutput)->depth(0,this,visited);
    if (cur_depth>max)
        max=cur_depth;
  }
